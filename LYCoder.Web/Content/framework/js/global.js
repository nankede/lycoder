...
$.fn.bindChangeEvent = function ($event) {
    ...
    $.fn.authorizeButton = function () {
        ...
        for (var i = 0; i < modules.length; i++) {
            if (modules[i].SPUrl != "") {
                if (modules[i].SPParentId == module.Id) {
        ...
        if (childModules.length > 0) {
            ...
            $.each(childModules, function (index, item) {
                _buttons += "<button id='" + item.SPEnCode + "' onclick='" + item.SPJsEvent + "' class=\"layui-btn layui-btn-primary layui-btn-small\">";
                _buttons += "   <i class='" + item.SPIcon + "' aria-hidden='true'></i> " + item.SPName + "";
                _buttons += "</button>";
            });
